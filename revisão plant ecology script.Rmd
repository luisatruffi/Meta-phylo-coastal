...
```{r}
setwd("~/Documents/mestrado/Meta analise/Analise/analises sem gimno git/Meta-phylo-coastal")
```
...
#codes for life form combinations
...
#codes for ontogenetic stage combination
...
##create effect size and es variance columns
...
#2- growth and growth_size  (53 all hedges)
...
#4- Survival (28 hedges d, 20 odds)
...
#aterpillars(model_geral, xlab = "Effect size (hedges g)" ,group = "study2022") 
...
2-a) Model phylogenetic distance
```{r}
...
Graph phylogenetic distance effect model:

```{r}
regplot(model.phylo.nogimno, xlab = "Phylogenetic distance", ylab = "Effect size (Hedges g)",  bty = "l")
```
2-b)models of phylogenetic distance effect for each performance variable:
...
```
Graphs phylogenetic distance for each performance variable:
...
Model phylogenetic distance*lifeform
```{r}
...
4- Model phylogenetic distance * ontogenetic stage combination
```{r}
...
lines(seq(0,650, length = 100),
      ...
      col =rgb(224, 201, 131, max = 255, alpha = 200))
```
5-a) Model phylogenetic distance * dune gradient
```{r}
...
lines(seq(0,650, length = 100),
      ...
      col =rgb(224, 201, 131, max = 255, alpha = 200))
```
5-b)Model phylogenetic distance *GAI
```{r}
...
phylogenetic distance*growth form 
...
#Review extra analysis
normality test
```{r}
plot(density(data$effectsize))

plot(density(data[c(which(data$lifestage_n_t == "notseedling_notseedling"| 
                      data$lifestage_n_t == "notseedling_seedling")),  "effectsize"]))
```

intraspecfic vs heterospecific
```{r}
data$intra <- 0

data[which(data$phylo_dist == 0), "intra"] <- 1
data$intra <- as.factor(data$intra)

model.intra.inter <-rma.mv(yi = effectsize, V= es_variance, random =(~1|study2022/outcome), 
                         mods = ~ 0 + intra,       data = data)

boxplot(formula = data$effectsize ~ data$intra)
```
